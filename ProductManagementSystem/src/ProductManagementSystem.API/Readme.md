...
WORKDIR /src
COPY ["ProductManagementSystem.API.csproj", "ProductManagementSystem.API/"]
RUN dotnet restore "ProductManagementSystem.API/ProductManagementSystem.API.csproj"

WORKDIR "/src/ProductManagementSystem.API"
COPY . .
RUN dotnet build "ProductManagementSystem.API.csproj" --configuration Release --output /app/build

FROM build AS publish
RUN dotnet publish "ProductManagementSystem.API.csproj" \
...
Execute unit tests
```
    cd "ProductManagementSystem.API/test/ProductManagementSystem.API.Tests"
    dotnet test
```

Deploy application
```
    cd "ProductManagementSystem.API/src/ProductManagementSystem.API"
